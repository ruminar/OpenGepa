...
Windows Menuの`.lnk`ショートカットは、通常アプリランチャーの編集画面へドラッグ＆ドロップで登録できます。ピン止めの有無は問いません。ドロップ先のGroupまたは行間へ、通常のFileItemとしてコピーされます。Windows Menu側のショートカットは変更せず、登録後の項目はWindows Menuと同期しません。同名がある場合は`_1`、`_2`のように自動改名します。
