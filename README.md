...
4. トレイアイコンを右クリックして［設定］を開き、Windows起動時の自動起動、外観、既定アイコン、項目の起動操作、縦タブの表示順を設定します。

ランチャーの右側にある縦タブを右クリックすると、そのランチャーの編集、複製、名前変更、アイコン変更、表示／非表示、削除を行えます。縦タブの空き領域を右クリックすると、設定とランチャーの新規登録を行えます。
...
- `appIcon1.png`、`appIcon2.png`、…: 新規ランチャー作成時に順番に使う初期アイコン
...
設定画面の［Profileを保存］と［Profileを読み込む］で、ランチャー構成とアイコンを`.ogp`ファイルとして移行できます。Profileには次の内容が入ります。
...
Profileを読み込むと、現在のランチャー設定は読み込んだ内容に置き換わります。Profileには実行ファイルやショートカットへの参照が含まれることがあるため、信頼できるProfileだけを読み込んでください。Windows自動起動の設定はPC固有のため、Profileには含まれません。
...
- v0.1.0ではDashboardの実コンテンツ、Windows Menu、Web専用タブ、ツリー内の区切り線、Undo／Redoは未対応です。
